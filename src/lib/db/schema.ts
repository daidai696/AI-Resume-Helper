...
export const jobApplications = sqliteTable('job_applications', {
  ...
  jobUrl: text('job_url'),
  jd: text('jd'),
  status: text('status', { enum: ['draft', 'applied', 'screening', 'interview', 'offered', 'rejected', 'accepted'] }).notNull().default('draft'),
  stage: text('stage'),
...
export const offers = sqliteTable('offers', {
  ...
  benefits: text('benefits', { mode: 'json' }).default('[]'),
...
// ===== 融合模块: 协作评论 =====
export const resumeComments = sqliteTable('resume_comments', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  resumeId: text('resume_id').notNull().references(() => resumes.id, { onDelete: 'cascade' }),
